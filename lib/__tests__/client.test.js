...
test('get bill api', () => {
  ...
  const billApi = client.getApi('bills');

  expect(billApi).toBeInstanceOf(bills);
});
...
test('get All Apis', () => {
  ...
  const allApis = client.getAllApis();

  expect(Object.keys(allApis).length).toBe(9);
});
